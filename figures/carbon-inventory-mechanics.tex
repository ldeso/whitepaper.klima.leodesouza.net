...
\node[market=steel_teal] (supply) at (-4,8.5) {Carbon Supply};

\node[token=steel_teal] (carbon1) at (-4,7) {\C{} Credit};
...
\node[token=sage_gray] (offset) at (4,5.5) {\CC{} Certificate};
\node[activity=warm_gray] (burn) at (4,4) {Burn \A{}};

\node[market=teal] (aam) at (0,2) {Carbon Inventory};

\node[token=teal] (atoken) at (-2,0) {\A{} Token};

\node[token=peacock_blue] (gtoken) at (2,0) {\G{} Token};

\node[activity=teal] (Inventory) at (-2,-1) {Inventory Selection};

\node[activity=peacock_blue] (capacity) at (2,-1) {Capacity};

\node[market=peacock_blue] (liquidity) at (-5,-3) {Liquidity Pools};
