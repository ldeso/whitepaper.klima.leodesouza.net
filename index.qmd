...
commercial use by Klima Foundation. No other party may implement, copy, modify,
or derive works from these materials without explicit written permission from
the author.
...
```{ojs}
viewof inputS_ = Inputs.range([1, 0], {
  label: tex`S`,
  step: 0.01,
  value: 0.5,
});
...
```{ojs}
viewof inputWeightAG = Inputs.range([1, 0], {
  label: tex`\text{Coefficient}`,
  step: 0.01,
  value: 0.5,
});
