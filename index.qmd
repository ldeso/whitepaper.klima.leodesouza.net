...
liquidity serve inputs into a coordination mechanism that adjusts protocol
...
Thus the Protocol is driven in response to its own native token allocations,
acting as rules-based carbon market infrastructure to connect available supply
with retirement demand. It is able to do so without using oracles or external
inputs, and without discretionary allocation, resale, or optimisation of
inventory.
...
**kVCM** allocations determines determines the protocol's execution rate for
carbon intake and retirement, expressed in **kVCM** units. These parameters
govern how the protocol issues or burns **kVCM** when carbon is supplied or
...
execution rateios, and the **K2** token modulating capacity.
...
1. **Carbon Suppliers & Retirees**
    ...
    **Carbon inventory**: Indicative excution rates for suppliers and retirees
...
continuous carbon retirement activity within the carbon markets.
...
a fixed duration. Lock durations are standardised at 90 days increments and
...
holders can allocate their token to carbon classes for inventory weighting.
...
allocations of **A** and **G** tokens creates a dynamic real-time execution rate
...
and the **A** tokens burned in connection with retirement is reflected solely as
...
As we have seen, a higher **G** token allocation $G_i$ for a carbon class
...
protocol. The number of liquid tonnes retired by the protocol AAM in exchange is
