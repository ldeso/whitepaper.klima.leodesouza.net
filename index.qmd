...
- define execution rates and intake eligibility for carbon credits against
...
execution rates, intake capacity, and participation incentives. Carbon credits
...
execution rates of **each class**, for both suppliers and retirees, by defining:
...
2. **Execution rate allocation**: Collective signalling of carbon classes via
**kVCM** allocations determines the protocol's execution rate for carbon intake
...
altering the execution rate, as defined by **kVCM** allocations.

Both tokens support the operation of the infrastructure, with **kVCM** informing
execution rates, and the **K2** token modulating capacity.
...
1. **Carbon Suppliers & Retirees**
    ...
    **Carbon inventory**: Indicative execution rates for suppliers and retirees
...
**C**&nbsp;(in) or carbon retirement certificates **C\***&nbsp;(out).
    ...
    are used in the protocol: allocations of **A** determine the execution rates
...
allocations of **A** and **G** tokens create a dynamic real-time execution rate
...
For execution terms, both **A** tokens and **G** tokens may be allocated to
specific carbon classes ${i \in \{1, 2, 3, \dots, n\}}$ and these are
...
materially altering the execution rate. As a consequence, the relationship
