...
3. **Liquidity Layer**:
    ...
    the case of **kVCM** with USDC, in order to generate liquidity fees in the
    standard way.
...
#### kVCM Incentives (Base Accrual) {#sec-kvcm-incentives}
...
The supply of **K2** is allocated at various rates, depending on overall system
balances:
...
The protocol's carbon inventory layer accumulates and retires carbon. It is
driven by parameters determined by its rules-based smart contracts, and user activities.
...
Thus the Protocol is driven in response to its own native token balances, acting
as rules-based carbon market infrastructure to connect available supply with
...
preferences within the system. Doing so may make them eligible for protocol
incentives when they perform defined coordination functions.
...
on the position's relative share of the pool.
...
- When **user-locked**:
    ...
    - It may vote for carbon classes to reduce the difference between execution
    terms on carbon intake and retirements.
...
allocations determines the protocol-defined execution capacity for carbon intake
and retirement operations for a given class. Higher capacity allocations
increase the system's ability to process additional carbon activity without
...
USDC tokens as **Q**, carbon credits as **C**, and carbon retirement certificates as
**C\***.

Three layers enable the Klima Protocol to find equilibrium through continuous,
rule-based feedback mechanisms representing system state&nbsp;(supply, demand,
coordinator allocations). There is no centralised management entity with
discretionary powers, or fees that can be turned on.

1. **Time-locking mechanics**: **A** token holders can time-lock their tokens
until a set date to influence system parameters and to have the ability to
select carbon classes for system weighting.

    - The collective locks influence the **A** incentives, as well as the
    real-time execution price for carbon credit acquisitions or retirements.

2. **Carbon inventory layer**: the protocol swaps **A** for carbon credits
...
the model, in the interests of their own use and preference. This, in
...
The incentives due on time-locked **A** tokens are calculated daily and added to
the locked balance, hence the daily accrual for each duration is calculated:
...
::: {#fig-growth-rate fig-cap="**A** annual growth rate from base accrual $\Delta S$." fig-alt="A annual growth rate from base accrual Delta S."}
...
#### Protocol Coordination Signals {#sec-protocol-governance-signals}

Protocol coordination signals can be derived from two participation cohorts:
...
allocations of **A** and **G** tokens creates a dynamic real-time execution
rate curve by carbon class.
...
balance, the amount of **A** tokens issued for carbon, $\Delta A$,
...
A retirement certificate for a carbon class with a zero **A** allocation cannot be
...
from the kVCM incentives calculation below. This is an [additional]{.underline}
...
#### kVCM Incentives {#sec-risk-premium-beta-determination}
...
#### Allocation of kVCM incentives {#sec-allocation-of-risk-premium}

The full issuance of **A** tokens is depicted below including now the kVCM 
incentives for the liquidity pools accordingly.
...
#### Share of kVCM incentives {#sec-share-of-risk-premium}
...
#### kVCM Incentives Distribution {#sec-risk-premium-distribution}
