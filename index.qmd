...
The supply of K2 is allocated at various rates, depending on
...
carbon assets, profit participation, or direct exposure to carbon price movements.
...
2. **Execution rate allocation**: Collective signalling of carbon classes
via **kVCM** allocations determines determines the protocol’s execution 
rate for carbonintake and retirement, expressed in kVCM units. These 
parameters govern how the protocol issues or burns kVCM when carbon is 
supplied or retired. Allocations may be updated over time.
...
allocations determines the protocol-defined execution capacity for carbon 
intake and retirement operations for a given class. Higher capacity 
allocations increase the system’s ability to process additional carbon activity 
without materially altering the execution rate, as defined by 
kVCM allocations.
...
1. **Carbon Suppliers & Retirees**
    ...
    **Carbon inventory**: Indicative excution rates for suppliers and retirees are
    continuously updated based on protocol state.
...
3. **Coordinators**
    ...
    **K2** tokens within pre-defined protocol parameters.
...
1. **Infrastructure, Not Extraction**:
    ...
    hidden spreads, or operate profit-taking mechanisms for any sponsor, foundation, 
    or investor. All protocol behaviour is rules-based and applies uniformly to all
    participants, with no privileged economic positions or revenue capture layers.
...
coordinator allocations). There is no centralised management entity with discretionary
powers, or fees that can be turned on.

1. **Time-locking mechanics**: **A** token holders can time-lock their tokens
until a set date to influence system parameters and to have the ability to select
carbon classes for system weighting.

    - The collective locks influence the **A** incentives, as well as the real-time
...
**C**&nbsp;(in) or carbon offset certificates **C\***&nbsp;(out).
    ...
    are used in the protocol: allocations of **A** determine the execution rates of
...
the model, in the interests of their own use and preference. This, in conjunction 
with the autonomous model, enables the protocol to fulfill continuous carbon
retirement activity within the carbon markets. 
...
user positions, via the 'time-weighted incentive curve'.
...
#### Time-weighted Incentive Curve {#sec-time-weighted-incentive-curve}
...
$$ {#eq-time-weighted-incentive-curve-shape}

Normalising $\gamma_t$ to $\hat \gamma_t$:

$$
\hat \gamma_t = \frac{\gamma_t}{\sum_{t=1}^{40} \gamma_t}
$$ {#eq-time-weighted-incentive-curve-normalised}
...
$$ {#eq-time-weighted-incentive-curve-normalized-sum}

The time-weighted incentive curve $Z_t$ is solved:

$$
Z_t = (1 - S) \, \frac{\Gamma_t}{E_t}
$$ {#eq-time-weighted-incentive-curve}
...
$$ {#eq-daily-time-weighted-incentives}
...
::::: {#fig-time-locked-incentives-example fig-cap="Example of time-locked incentives."}
...
```{ojs}
//| fig-cap: Time-weighted incentives (${stringS}, ${stringI}).
Plot.plot({
  color: {
    legend: true,
...
![Time-weighted incentives (Total Stake = 55.00%, Inflation = 1.75%).](figures/time-locked-incentives-example-curve.svg){
  #fig-time-locked-incentives-example-curve
  fig-alt="Time-weighted incentives (Total Stake = 55.00%, Inflation = 1.75%)."
}

![Discount rates.](figures/time-locked-incentives-example-discount-rates.svg){
  #fig-time-locked-incentives-example-discount-rates
...
staking and duration, @fig-inflation-rate assumes a single duration over the
staking range to provide an approximation of inflation
$\Delta S \approx Z \, S$.

::: {#fig-inflation-rate fig-cap="**A** inflation rate from base accrual $\Delta S$." fig-alt="A inflation rate from base accrual Delta S."}
...
1. Time-locked **A** & user-locked **G** tokens
    ...
    previously in @sec-time-weighted-incentive-curve):
